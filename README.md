...
A personal movie and TV show tracking system built on PostgreSQL. Instead of maintaining manual lists, I built a proper relational database that automates the tedious parts: status updates, rewatch detection, audit logging, and feeds clean data directly into Power BI dashboards.

A Python pipeline (kept private) handles TMDB API integration and calls the stored procedures to write data into the database.
...
A private Python pipeline sits between the TMDB API and the database. It fetches structured metadata (genres, runtime, episode counts, TMDB IDs), validates and cleans the data, then calls the appropriate stored procedure to write it to the database. Credentials are managed via a `.env` file and never committed to version control.
